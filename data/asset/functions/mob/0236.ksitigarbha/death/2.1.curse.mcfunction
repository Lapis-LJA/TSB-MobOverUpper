...
# playerにはcore側でFriendが自動で付与されている
    ...
    execute as @e[type=#lib:living,tag=Friend,tag=!Uninterferable,distance=..20] run function api:damage/
...
# Duration = 10(17 + 3 * Difficulty)
    ...
    execute as @e[type=#lib:living,tag=Friend,tag=!Uninterferable,distance=..20] run function api:entity/mob/effect/give
    function api:entity/mob/effect/reset

# 毒
# Stack = Difficulty
    data modify storage api: Argument set value {ID:29,Duration:100}
    execute store result storage api: Argument.Stack int 10 run data get storage api: Return.Difficulty
    execute as @e[type=#lib:living,tag=Friend,tag=!Uninterferable,distance=..20] run function api:entity/mob/effect/give
    function api:entity/mob/effect/reset

# 音
    playsound entity.elder_guardian.curse hostile @a[tag=!PlayerShouldInvulnerable,distance=..20] ~ ~ ~ 1 1 1

# メッセージ
    execute as @e[tag=!PlayerShouldInvulnerable,distance=..20] run tellraw @s [{"translate": "%1$sはお地蔵さんを攻撃した罰が当たった。","with":[{"selector":"@s"}]}]
