...
# 与えるダメージ
    ...
    data modify storage api: Argument.DeathMessage append value '{"translate": "%1$sは%2$sによってべとべとに斬り裂かれた","with":[{"selector":"@s"},{"nbt":"Return.AttackerName","storage":"lib:","interpret":true}]}'
...
# 採掘速度低下
    data modify storage api: Argument set value {ID:26,Duration:160}
...
# 毒
    data modify storage api: Argument set value {ID:29,Duration:160}
    execute store result storage api: Argument.Stack int 2 run data get storage api: Return.Difficulty
    execute as @p[tag=Victim] run function api:entity/mob/effect/give
    function api:entity/mob/effect/reset

# 与ダメージ低下
    data modify storage api: Argument set value {ID:1,Duration:160}
    execute store result storage api: Argument.Stack int 2 run data get storage api: Return.Difficulty
    execute as @p[tag=Victim] run function api:entity/mob/effect/give
    function api:entity/mob/effect/reset
