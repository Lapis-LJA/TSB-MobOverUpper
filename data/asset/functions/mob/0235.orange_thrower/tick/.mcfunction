...
# Tickの処理順の関係で0以上の方を先に書く
    ...
    execute if entity @s[scores={General.Mob.Tick=..0}] if entity @p[distance=..16] run scoreboard players add @s General.Mob.Tick 1

# スコアが0の時、周囲にプレイヤーがいなければスコアを戻す
    execute if entity @s[scores={General.Mob.Tick=0}] unless entity @p[distance=..16] run scoreboard players set @s General.Mob.Tick -60
