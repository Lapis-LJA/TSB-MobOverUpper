...
# (Difficulty * 2 + 1)
    ...
    execute store result score $AttackCount Temporary run data get storage api: Return.Difficulty 3
...
# プレイヤーが4m以上離れている場合、プレイヤーの方向に召喚
    execute if predicate api:global_vars/difficulty/max/2_hard if entity @p[gamemode=!spectator,distance=..4] at @p[gamemode=!spectator,distance=..4] run function api:object/summon
    execute if predicate api:global_vars/difficulty/max/2_hard if data storage api: Argument.ID if entity @p[gamemode=!spectator,distance=4..30] facing entity @p[gamemode=!spectator,distance=4..30] feet positioned ^ ^ ^4 run function api:object/summon
...
# 次に攻撃するタイミングをランダムにする
    execute store result score @s General.Mob.Tick run random value -90..-70
