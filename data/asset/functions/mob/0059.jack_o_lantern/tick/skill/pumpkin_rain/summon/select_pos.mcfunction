...
# ハード以上なら (座標 or 偏差) + エリア内拡散で召喚
    ...
    execute if entity @s[tag=!1N.Random] at @a[distance=..50] run function asset:mob/0059.jack_o_lantern/tick/skill/pumpkin_rain/summon/summon
    execute if entity @s[tag=1N.Random] at @a[distance=..50] run function asset:mob/0059.jack_o_lantern/tick/skill/pumpkin_rain/summon/predict_set
